...
::spawnHeldCube <- async(function (pos = null, targetname = null) {
  ...
  printl("Spawned cube with entindex " + cube.entindex());
...
/**
 * Destroys the cube currently held by the player.
 * @returns {bool} True when a held cube was found and destroyed.
 */
::destroyHeldCube <- async(function () {

  if (!pplayer.holding()) return false;

  local player = GetPlayer();
  local cube = null;

  while (cube = Entities.FindByClassname(cube, "prop_weighted_cube")) {
    if (cube.GetMoveParent() == player) {
      cube.Destroy();
      return true;
    }
  }

  printl("Cube has been destroyed!");
  return false;

});

/**
 * Monitors a cube and calls a callback function when it's destroyed.
 * @param {entity} cube - The cube entity to monitor.
 * @param {function} callback - The callback function to call when the cube is destroyed.
 * @returns {ppmod.interval} An interval handle that can be used to stop monitoring.
 */
::monitorCube <- function (cube, callback) {

  if (!ppmod.validate(cube)) throw "monitorCube: Invalid cube entity";
  if (typeof callback != "function") throw "monitorCube: Invalid callback function";

  local entindex = cube.entindex();

  return ppmod.interval(function (args):(callback, entindex) {
    if (!args.cube.IsValid()) {
      printl("Cube " + entindex + " has been destroyed");
      callback(entindex);
...
/**
 * Spawns a cube and monitors it for destruction, reporting when it's destroyed.
 * @param {Vector|null} pos - The position to spawn the cube at, or null to spawn at the player's feet.
 * @param {string|null} targetname - The targetname for the cube, or null if not needed.
 * @param {function|null} callback - Optional callback function to call when cube is destroyed.
 * @returns {ppromise<table>} A promise that resolves with a table containing cube and monitor info.
 */
::spawnAndMonitorCube <- async(function (pos = null, targetname = null, callback = null) {

  // Spawn the cube
  yield spawnHeldCube(pos, targetname);
  local cube = yielded;

  if (!cube) return null;

  // Set up the callback
  local onDestroy = callback != null ? callback : function (entindex) {
    printl("Cube was destroyed");
  };

  // Start monitoring the cube
  local monitor = monitorCube(cube, onDestroy);

  return {
    cube = cube,
    monitor = monitor,
    entindex = cube.entindex()
  };
